...
class TaskManager {
  ...
  addTask = (name, description, assignee, dueDate, status) => {
    ...
    if (validateInput()) {
      // Getting the data from the Local storage after checking array not empty
      if (localStorage.getItem('task')) {
        JSON.parse(localStorage.getItem('task')).forEach(element => {
          console.log(element)
          // Getting the last element id ,  set to the next task Id
      ...
      // Pushing Task Details into the Array
      this._taskArray.push(task);
      // Setting up Local storage and Inserting data what is the Array
  ...
  displayTasks = () => {
    ...
    // checking for data not empty and looping through objects to pass the data to HTML elements
